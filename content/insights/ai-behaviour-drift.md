...
OpenAI's own history makes the point in a more human way. In April 2025 it shipped an update that made the model so eager to please it would agree with almost anything put to it, then [rolled the update back within days](https://openai.com/index/sycophancy-in-gpt-4o/) once users noticed. The lesson is the same in both cases. The model your team signed off on is a snapshot. The live service is a moving target, and the movement is usually silent.
