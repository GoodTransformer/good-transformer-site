...
| `NEXT_PUBLIC_LINKEDIN_PARTNER_ID` | Recommended | LinkedIn Insight Tag partner ID from the Good Transformer LinkedIn Campaign Manager ad account. Current value: `9252146`. Used for website visitor audience insights, retargeting, and conversion optimisation. |

These use the `NEXT_PUBLIC_` prefix because they are embedded in client-side JavaScript. They are safe to expose - they are public form endpoints, public booking links, a contact email, public analytics/tracking IDs, not secret keys.
...
| `NEXT_PUBLIC_LINKEDIN_PARTNER_ID` | `src/components/linkedin-insight-tag.tsx` via the root layout |
...
| `NEXT_PUBLIC_LINKEDIN_PARTNER_ID` | Same, if you want LinkedIn Insight Tag visitor insights and retargeting in production |
